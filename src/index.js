...
const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url.startsWith('/cats/edit-cat/')) {
        ...
        req.on("end", () => {
            ...
            console.log(catId, catName, catImageUrl, catDescription, catBreed);
            if (catIndex !== -1) {
    ...
    if (req.url === '/') {
        ...
        if (cat) {
            ...
            htmlContent = htmlContent.replace('{{breeds}}', breeds.map(breed => breed!==cat.breed ? `<option value="${breed.name}">${breed.name}</option>` : `<option value="${breed.name}" selected>${breed.name}</option>`).join('\n'));
        } else {
            res.writeHead(404);
            res.end();
            return;
        ...
        htmlContent = htmlContent.replace('{{catId}}', catId)
